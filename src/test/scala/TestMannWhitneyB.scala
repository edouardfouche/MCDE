...
import com.edouardfouche.stats.mcde.MWB
import org.scalatest.FunSuite

/**
  * Created by fouchee on 10.07.17.
  */
class TestMannWhitneyB extends FunSuite with TestData {
  val test = MWB(1000, 0.5)
  ...
  test("Check that MWB values are always between 0 and 1, alpha=0.1") {
    val preprocessed = MWB(1, 0.1).preprocess(lowcontrast_2D)
    val res = for {x <- 0 until 10000} yield MWB(1, 0.1).contrast(preprocessed, dimensions = Set(0, 1))
    assert(!res.exists(x => x < 0 | (x-1 > 0.001)))
  }
  test("Check that MWB values are always between 0 and 1, alpha=0.5") {
    val preprocessed = MWB(1, 0.1).preprocess(lowcontrast_2D)
    val res = for {x <- 0 until 10000} yield MWB(1, 0.5).contrast(preprocessed, dimensions = Set(0, 1))
    assert(!res.exists(x => x < 0 | (x-1 > 0.001)))
  }
  test("Check that MWB values are always between 0 and 1, alpha=0.9") {
    val preprocessed = MWB(1, 0.1).preprocess(lowcontrast_2D)
    val res = for {x <- 0 until 10000} yield MWB(1, 0.9).contrast(preprocessed, dimensions = Set(0, 1))
    assert(!res.exists(x => x < 0 | (x-1 > 0.001)))
  }
