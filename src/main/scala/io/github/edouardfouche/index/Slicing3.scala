...
/**
  ...
  */
object Slicing3 extends Slicer[(Int, Float, Double)]{
  /**
  ...
  def randomSlice(m: S, dimensions: Set[Int], referenceDim: Int, sliceSize: Int): Array[Boolean] = {
    ...
    //println(s"m(0).length : ${m(0).length}")
  ...
  // UniformSlice has the particularity to alleviate border effects.
  // That is to say, every objects have the same contribution in the contrast estimation
  def randomUniformSlice(m: S, dimensions: Set[Int], referenceDim: Int, sliceSize: Int): Array[Boolean] = {
    //require(dimensions.forall(x => x>=0 & x < m.length), "The dimensions for deviation need to be greater or equal to 0 and lower than the total number of dimensions")
    //require(dimensions.contains(referenceDim), "The reference dimensions should be contained in the set of dimensions")
    //require(sliceSize > 0, "sliceSize should be greater than 0")
    ...
    for {dim <- dimensions.filter(_ != referenceDim)} {
      ...
      if(flag == 1) {
        ...
        for {x <- sliceStart until sliceStart + counterSliceSize} {logicalArray(m(dim)(x)._1) = false}
      }
    }
    logicalArray
  }
  ...
  def safeSlice(m: S, dimensions: Set[Int], referenceDim: Int, sliceSize: Int): Array[Boolean] = {
    ...
    val logicalArray = Array.fill[Boolean](m(0).length)(true)
    for {dim <- dimensions.filter(_ != referenceDim)} {
      val sliceStart = getSafeCut(scala.util.Random.nextInt((m(0).length - sliceSize).max(1)), m, dim)
      val sliceEnd = getSafeCut(sliceStart + sliceSize, m, dim)
      for {x <- 0 until sliceStart} {logicalArray(m(dim)(x)._1) = false}
      for {x <- sliceEnd until m(0).length} {logicalArray(m(dim)(x)._1) = false}
    }
    ...
    while (logicalArray.count(_ == true) > sliceSize) {
      val nrep = logicalArray.count(_ == true) - sliceSize
      for {
        x <- 0 to nrep
      } {
  ...
  // it has a backtracking step to "ensure" that the slice has not too much element (not sure whether this is a good idea actually)
